...
def requests_users(request, enpoint: Literal['update/', 'verify/', 'delete/'], method: Literal['GET', 'PUT', 'PATCH', 'DELETE'], data=None):
    if request is None:
    ...
    try:
        response = requests.request(
            method=method,
            url='http://auth:8005/api/users/' + enpoint,
            headers={
                'Authorization': token,
...
def validate_username(request, username):
    return requests_users(request, f'validate/chat/', 'GET')
